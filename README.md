...
- **免费视觉后端**：默认视觉模型改为 SiliconFlow 的 `Qwen/Qwen3-VL-8B-Instruct`（免费），无需付费 DashScope
- **独立 CLI 工具**：新增 [`vision-cli.py`](./vision-cli.py)，无需启动代理服务，命令行直接识图，支持环境变量 / 配置文件 / 交互输入三种 Key 传入方式
- **端口调整**：默认端口改为 `8001`
...
启动后访问 `http://localhost:8001`，在配置器页面填写 API Key，点击「应用并重启」即可。
...
默认使用 **SiliconFlow 免费视觉模型** `Qwen/Qwen3-VL-8B-Instruct`，只需填写 `VISION_API_KEY` 即可启用（[免费注册获取](https://cloud.siliconflow.cn)）：
...
- SiliconFlow（`Qwen/Qwen3-VL-8B-Instruct` 等，免费额度）
...
不想启动代理服务？直接用 `vision-cli.py` 在命令行识图：

```bash
# 首次使用会提示输入 API Key（自动保存到 ~/.vision_key，免重复输入）
python vision-cli.py screenshot.png
